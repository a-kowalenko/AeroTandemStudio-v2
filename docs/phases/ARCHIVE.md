# Phasen-Archiv (erledigte Specs)

> **Nicht** standardmäßig an Agent-Kontext anhängen.
> Nur gezielt öffnen (Regression, Nachschlagen, abgeschlossene Unterphasen).
>
> Offene Arbeit: `docs/phases/open/`
> Index / Tracker: `docs/IMPLEMENTATION_PLAN.md`

---
## 8. Phasenplan

> **Anleitung:** Kopiere den Prompt der jeweiligen Phase in ein neues Agent-Fenster.
> Offene Phasen: `@docs/phases/open/…`. Erledigte Specs stehen hier im Archiv — nur bei Bedarf anhängen.
...
> **Aktuelle Spec (offen):** `docs/phases/open/14-ml-photo.md`
...
> **Aktuelle Spec (offen / in Arbeit):** `docs/phases/open/23-usb-mtp.md` — Agents bitte diese Datei attachen.
...
> **Aktuelle Spec (offen):** `docs/phases/open/31.5-extra-files.md` — Agents bitte diese Datei attachen.
...
### Phase 41 — Eject-Erfolgs-Ton (SD/MTP)

**Status:** ✅ Erledigt  
**Abhängigkeiten:** Phase 7 (SD-Eject), Phase 23 (MTP soft-eject), `showSdEjectToast`  
**Ziel:** Optionaler Sound (Default **an**), wenn SD-Karte oder MTP-Gerät **erfolgreich** ausgeworfen wurde — Signal „jetzt sicher ziehen“, auch wenn der Operator nicht auf die App schaut.

> Eine Agent-Session = nur Phase 41. Kein Insert-/Backup-Sound, kein Fehlerton, kein Setup-Wizard, kein Lautstärke-Slider.

#### Entscheidungen

| # | Thema | Entscheidung |
|---|--------|----------------|
| 1 | Trigger | Nur bei **erfolgreichem** Eject (`ok: true`). **Nie** bei Fehler — Toast reicht („bitte manuell entfernen“). |
| 2 | Pfade | **Jeder** erfolgreiche Eject: Auto-Workflow **und** manuell (`SdDriveSelector`). Ein Hook reicht. |
| 3 | Fokus | **Nicht** an Fenster-Fokus koppeln — Ton soll gerade dann helfen, wenn der Operator woanders schaut. |
| 4 | Hook | Einziger Aufruf in `showSdEjectToast` bei `ok === true` (nach Config-Check). Keine Doppel-Logik in `App.tsx` / `SdDriveSelector`. |
| 5 | Audio | Frontend: **eine** wiederverwendete `HTMLAudioElement`-Instanz. Bei Doppel-Aufruf kurz **debouncen** (Analog zur Toast-`id` `sd-eject-…-ok`). |
| 6 | Fehler | `play().catch(() => {})` — Audio darf Toast/Setting/Workflow **nie** abbrechen. |
| 7 | Config | `sd_eject_sound_enabled: bool`, Default **`true`**. Merge-with-defaults wie andere SD-Flags. |
| 8 | UI | Settings → SD → Import-Sektion, direkt unter „SD automatisch auswerfen“. Kein Setup-Wizard. |
| 9 | Asset | Bestehende `.mp3` nach `public/sounds/eject-ok.mp3` (URL `/sounds/eject-ok.mp3`). |
| 10 | Plattform | Web Audio im Tauri-WebView; Autoplay nach Workflow einmal manuell Win/macOS/Linux prüfen. |

#### Out of Scope

- Fehlerton / Warning-Sound
- Ton bei physischem Abziehen ohne Eject
- Unterschiedliche Sounds SD vs. MTP
- Lautstärke-Slider / System-Sound-API
- Setup-Wizard-Eintrag
- Native OS-Benachrichtigungston

#### Scope / Checkliste

- [x] Config: `sd_eject_sound_enabled` in `config.rs` (+ Default `true`) und `AppConfig` in `tauri.ts`
- [x] Settings: Checkbox + Hint in `SdTab.tsx` (Import-Sektion)
- [x] i18n de / en / es-MX (`settings.sd.import.ejectSound` + Hint)
- [x] Asset: `public/sounds/eject-ok.mp3`
- [x] Helper z. B. `src/lib/ejectSound.ts`: Singleton-Audio, Debounce, silent catch; liest Config (Store/`getConfig`-Snapshot — gleiches Muster wie andere UI-Flags)
- [x] `showSdEjectToast`: bei `ok` und Setting an → `playEjectSound()`; bei `!ok` **kein** Sound
- [x] Config-Schema §9 + Tracker in diesem Plan; `AGENTS.md` „Nächster Schritt“ optional
- [x] Default-Test in `config.rs` (`assert!(cfg.sd_eject_sound_enabled)`)
- [x] `cargo test` + `npm run check`

#### Akzeptanzkriterien

1. Frische Config / Factory: `sd_eject_sound_enabled === true`.
2. Erfolgreicher Auto-Eject → Toast **und** Ton (auch wenn Fenster nicht fokussiert).
3. Erfolgreicher manueller Eject → gleicher Ton.
4. Fehlgeschlagenes Eject → Toast fail, **kein** Ton.
5. Setting aus → nur Toast, kein Ton.
6. Doppelter Success-Aufruf kurz hintereinander → kein doppelter „Stotter“-Ton (Debounce).
7. Audio-Fehler (fehlende Datei / Autoplay-Block) → Workflow und Toast bleiben intakt.

#### Referenzen

```
src/lib/sdEjectToast.tsx
src/lib/ejectSound.ts          # NEU
public/sounds/eject-ok.mp3     # NEU (vom Operator bereitgestelltes SFX)
src/components/settings/tabs/SdTab.tsx
src/components/SdDriveSelector.tsx   # nur indirekt via Toast
src/App.tsx                          # nur indirekt via Toast
src-tauri/src/storage/config.rs
src/lib/tauri.ts
src/locales/de.json | en.json | es-MX.json
```
...
```
Implementiere Phase 41 aus @docs/IMPLEMENTATION_PLAN.md
Regeln: @AGENTS.md
Nur Phase 41 (Eject-Erfolgs-Ton).
Sound nur bei ok=true in showSdEjectToast; Debounce + silent catch; Default on.
Kein Fehlerton, kein Wizard, kein Lautstärke-Slider.
Danach cargo test && npm run check.
```
