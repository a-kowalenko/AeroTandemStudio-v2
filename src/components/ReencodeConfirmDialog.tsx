...
function reasonLabel(
  ...
  const key = `dialogs.reencode.reasons.${kind}`;
  const translated = t(key);
...
export function ReencodeConfirmDialog({
  ...
  const description = reasonLabel(t, kind, reason);
  ...
  const summary = [
    ...
    t(videoCrfLabelKey(profile.crf)),
    profile.hw_accel
          ...
          {advancedOpen ? (
            <div className="grid gap-2.5 rounded-md border border-border/50 p-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={String(nearestVideoCrf(profile.crf))}
                  onValueChange={(v) =>
                    patchProfile({ crf: Number(v) })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
