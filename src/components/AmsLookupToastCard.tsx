...
export function AmsLookupToastCard({
  visible,
  title,
  name,
  media,
    ...
    >
      ...
      <div className="flex items-start gap-3 py-3.5 pr-3 pl-4">
        ...
        <div className="min-w-0 flex-1 pt-0.5">
          ...
          <p className="mt-1 text-[12.5px] leading-snug text-muted">{name}</p>
          {media?.trim() ? (
            <p
              className={cn(
                "mt-2 inline-flex max-w-full items-center truncate rounded-md px-1.5 py-0.5",
                "bg-foreground/[0.06] text-[11px] font-medium tracking-wide text-foreground/80",
              )}
              title={media}
            >
              {media.trim()}
