...
| Rust `processor.rs` | `build_body_reencode_output_tail(out_codec, audio_is_aac)` — pix_fmt/Audio/faststart (rein, getestet) |
...
- `build_body_reencode_output_tail` (H.264 → yuv420p + AAC-Copy; HEVC → kein pix_fmt + Transcode)
